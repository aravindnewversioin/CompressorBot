...
async def encod(event):
    try:
        ...
        pp = []
        async for x in event.client.iter_participants("@aravindbots"):
           pp.append(x.id)
        if (user.id) not in pp:
           return await xxx.edit(
               "You must SUBSCRIBE this channel to use this bot",
              buttons=[Button.url("Let's Join Channel", url="@aravindbots")],
           )
